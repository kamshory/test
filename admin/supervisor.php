...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	try
	{
		...
		$supervisor->setPassword($passwordHash2);
	}
	catch(Exception $e)
	{
		// do nothing
	...
	$supervisor->setNama(trim($inputPost->getNamaDepan(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true).' '.$inputPost->getNamaBelakang(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true)));
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	try
	{
		$specs = PicoSpecification::getInstance()
			->addAnd(PicoPredicate::getInstance()->equals(Field::of()->supervisorId, $inputPost->getSupervisorId()))
			->addAnd(PicoSpecification::getInstance()->addOr(PicoPredicate::getInstance()->equals(Field::of()->waitingFor, WaitingFor::NOTHING))->addOr(PicoPredicate::getInstance()->equals(Field::of()->waitingFor, null)))
			;
		...
		$supervisorApv = new SupervisorApv($supervisor, $database); // SupervisorApv is the approval entity for Supervisor

		// Update the SupervisorApv entity
		...
		if(!empty($password))
		{
			try
			{
				$passwordHash1 = sha1($password);
				$passwordHash2 = sha1($passwordHash1);
				$supervisorApv->setPassword($passwordHash2);
			}
			catch(Exception $e)
			{
				// do nothing
		...
		$supervisorApv->setNama(trim($inputPost->getNamaDepan(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true).' '.$inputPost->getNamaBelakang(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true)));
		...
		$supervisorApv->setSupervisorId($inputPost->getSupervisorId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
...
else if($inputPost->getUserAction() == UserAction::APPROVE)
{
	if($inputPost->issetSupervisorId())
	{
		...
		if($supervisor->issetSupervisorId())
		{
			...
			$approvalCallback->setAfterInsert(function($param1 = null, $param2 = null, $param3 = null) use ($dataControlConfig, $database, $currentAction, $appConfig) {
				// Logic to execute after new data is inserted
				// Your code goes here

				$userLevelId = $appConfig->getSupervisorLevelId();

				$param1->load(); // Load the Supervisor data from the database

				// Create specification to find the Supervisor data with the specified Supervisor ID
				$specs1 = PicoSpecification::getInstance()
					->addAnd(PicoPredicate::getInstance()->equals(Field::of()->supervisorId, $param1->getSupervisorId()))
					;
				...
				try
				{
					...
					// Found
					$admin->setUsername($param1->getUsername());
					$admin->setPassword($param1->getPassword());
					$admin->setNamaDepan($param1->getNamaDepan());
					$admin->setNamaBelakang($param1->getNamaBelakang());
					$admin->setNama($param1->getNama());
					$admin->setUmkId($param1->getUmkId());
					$admin->setTskId($param1->getTskId());
					$admin->setEmail($param1->getEmail());
					$admin->setTelepon($param1->getTelepon());
					$admin->setJenisKelamin($param1->getJenisKelamin());
					$admin->setTipePengguna("supervisor");
					$admin->setUserLevelId($userLevelId);
					$admin->setSupervisorId($param1->getSupervisorId());
					$admin->setAktif($param1->getAktif());
					$admin->setBlokir($param1->getBlokir());
			...
			$approvalCallback->setBeforeUpdate(function($param1 = null, $param2 = null, $param3 = null) use ($dataControlConfig, $database, $currentAction, $appConfig) {
				// Logic to execute before updating data
				// Your code goes here
				
			});

			$approvalCallback->setAfterUpdate(function($param1 = null, $param2 = null, $param3 = null) use ($dataControlConfig, $database, $currentAction, $appConfig) {
