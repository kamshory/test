...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->jenisHariLiburId, $inputPost->getJenisHariLiburId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT));
	$specification->addAnd($dataFilter);
	$jenisHariLibur = new JenisHariLibur(null, $database);
	$updater = $jenisHariLibur->where($specification)
	...
	{
		$updater->update();
		$newId = $inputPost->getJenisHariLiburId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT);
		$currentModule->redirectTo(UserAction::DETAIL, Field::of()->jenis_hari_libur_id, $newId);
	}
	catch(Exception $e)
	{
		...
		foreach($inputPost->getCheckedRowId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT) as $rowId)
		{
			try
			{
				$specification = PicoSpecification::getInstance()
					...
					;
				$jenisHariLibur = new JenisHariLibur(null, $database);
				$jenisHariLibur->where($specification)
					->delete();
			}
			catch(Exception $e)
			{
...
{
$appEntityLanguage = new AppEntityLanguageImpl(new JenisHariLibur(), $appConfig, $currentUser->getLanguageId());
...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->jenisHariLiburId, $inputGet->getJenisHariLiburId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT));
	$specification->addAnd($dataFilter);
	$jenisHariLibur = new JenisHariLibur(null, $database);
	try{
		$jenisHariLibur->findOne($specification);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td>
							<input type="number" step="1" class="form-control" name="sort_order" id="sort_order" value="<?php echo $jenisHariLibur->getSortOrder();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		...
		"waktuUbah" => array(
			...
			"entityName" => "WaktuUbah",
			"tableName" => "waktu_ubah",
			"primaryKey" => "waktu_ubah",
			"objectName" => "waktu_ubah",
			"propertyName" => "nama"
		)
		);
		$jenisHariLibur->findOne($specification, null, $subqueryMap);
		if($jenisHariLibur->issetJenisHariLiburId())
		{
$appEntityLanguage = new AppEntityLanguageImpl(new JenisHariLibur(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $jenisHariLibur->dateFormatWaktuBuat('j F Y H:i:s');?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminBuat();?></td>
						<td><?php echo $jenisHariLibur->getAdminBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminUbah();?></td>
						<td><?php echo $jenisHariLibur->issetPengubah() ? $jenisHariLibur->getPengubah()->getNama() : "";?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getWaktuUbah();?></td>
						<td><?php echo $jenisHariLibur->issetWaktuUbah() ? $jenisHariLibur->getWaktuUbah()->getNama() : "";?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpBuat();?></td>
						<td><?php echo $jenisHariLibur->getIpBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpUbah();?></td>
						<td><?php echo $jenisHariLibur->getIpUbah();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAktif();?></td>
				...
				<div class="data-wrapper">
					<table class="table table-row table-sort-by-column">
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $jenisHariLibur->getJenisHariLiburId();?>" data-sort-order="<?php echo $jenisHariLibur->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>" data-active="<?php echo $jenisHariLibur->optionAktif('true', 'false');?>">
								<?php if($userPermission->isAllowedSortOrder()){ ?>
