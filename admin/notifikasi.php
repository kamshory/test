		...
		foreach($inputPost->getCheckedRowId() as $rowId)
		{
			...
			try
			{
				$notifikasi->where(PicoSpecification::getInstance()
					...
					->addAnd(PicoPredicate::getInstance()->equals(Field::of()->adminId, $currentUser->getUserId()))
				)
				->setDibaca(true)
				->update();
			}
...
if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		...
		{
$appEntityLanguage = new AppEntityLanguageImpl(new Notifikasi(), $appConfig, $currentUser->getLanguageId());
...
$specification->addAnd(PicoPredicate::getInstance()->equals('tipePengguna', 'user'));
