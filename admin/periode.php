...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$periode->setDefaultData($inputPost->getDefaultData(PicoFilterConstant::FILTER_SANITIZE_BOOL, false, false, true));
...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->periodeId, $inputPost->getPeriodeId(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS));
	$specification->addAnd($dataFilter);
	$periode = new Periode(null, $database);
	$updater = $periode->where($specification)
	...
	try
	{
		...
		$periode = new Periode(null, $database);
		$periode->where($specification)->setPeriodeId($newId)->update();
		$currentModule->redirectTo(UserAction::DETAIL, Field::of()->periode_id, $newId);
	}
	catch(Exception $e)
	{
...
{
$appEntityLanguage = new AppEntityLanguageImpl(new Periode(), $appConfig, $currentUser->getLanguageId());
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						...
						<td>
							<input type="text" class="form-control" name="periode_id" id="periode_id" autocomplete="off"/>
						</td>
					</tr>
					<tr>
...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->periodeId, $inputGet->getPeriodeId(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS));
	$specification->addAnd($dataFilter);
	$periode = new Periode(null, $database);
	try{
		$periode->findOne($specification);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getPeriodeId();?></td>
						<td>
							<input type="text" class="form-control" name="app_builder_new_pk_periode_id" id="periode_id" value="<?php echo $periode->getPeriodeId();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
						<td>
							<input type="text" class="form-control" name="nama" id="nama" value="<?php echo $periode->getNama();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		$subqueryMap = null;
		$periode->findOne($specification, null, $subqueryMap);
		if($periode->issetPeriodeId())
		{
$appEntityLanguage = new AppEntityLanguageImpl(new Periode(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $periode->dateFormatWaktuBuat('j F Y H:i:s');?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpBuat();?></td>
						<td><?php echo $periode->getIpBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminBuat();?></td>
						<td><?php echo $periode->getAdminBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getWaktuUbah();?></td>
						<td><?php echo $periode->dateFormatWaktuUbah('j F Y H:i:s');?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpUbah();?></td>
						<td><?php echo $periode->getIpUbah();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminUbah();?></td>
						<td><?php echo $periode->getAdminUbah();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAktif();?></td>
			...
			<form action="" method="get" class="filter-form">
				<span class="filter-group">
					...
					<span class="filter-control">
						<input type="text" class="form-control" name="periode_id" value="<?php echo $inputGet->getPeriodeId();?>" autocomplete="off"/>
					</span>
				</span>
				...
				<div class="data-wrapper">
					<table class="table table-row table-sort-by-column">
						...
						<tbody data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>" data-active="<?php echo $periode->optionAktif('true', 'false');?>">
								<?php if($userPermission->isAllowedBatchAction()){ ?>
