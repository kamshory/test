...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/admin", "kelas-tower", $appLanguage->getKelasTower());
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$kelasTower->setDefaultData($inputPost->getDefaultData(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$kelasTower->setAktif($inputPost->getAktif(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$kelasTower->setAdminBuat($currentAction->getUserId());
	$kelasTower->setWaktuBuat($currentAction->getTime());
	$kelasTower->setIpBuat($currentAction->getIp());
	$kelasTower->setAdminUbah($currentAction->getUserId());
...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->kelasTowerId, $inputPost->getKelasTowerId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT));
	$specification->addAnd($dataFilter);
	$kelasTower = new KelasTower(null, $database);
	$updater = $kelasTower->where($specification)
		...
		->setDefaultData($inputPost->getDefaultData(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true))
		->setAktif($inputPost->getAktif(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true))
	;
	...
	{
		$updater->update();
		$newId = $inputPost->getKelasTowerId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT);
		$currentModule->redirectTo(UserAction::DETAIL, Field::of()->kelas_tower_id, $newId);
	}
	catch(Exception $e)
	{
		...
		foreach($inputPost->getCheckedRowId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT) as $rowId)
		{
			try
			{
				$specification = PicoSpecification::getInstance()
					...
					;
				$kelasTower = new KelasTower(null, $database);
				$kelasTower->where($specification)
					->delete();
			}
			catch(Exception $e)
			{
...
{
$appEntityLanguage = new AppEntityLanguageImpl(new KelasTower(), $appConfig, $currentUser->getLanguageId());
...
{
	$specification = PicoSpecification::getInstanceOf(Field::of()->kelasTowerId, $inputGet->getKelasTowerId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT));
	$specification->addAnd($dataFilter);
	$kelasTower = new KelasTower(null, $database);
	try{
		$kelasTower->findOne($specification);
		if($kelasTower->issetKelasTowerId())
		{
$appEntityLanguage = new AppEntityLanguageImpl(new KelasTower(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
?>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
						<td>
							<input type="text" class="form-control" name="nama" id="nama" value="<?php echo $kelasTower->getNama();?>" autocomplete="off" required="required"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		$subqueryMap = null;
		$kelasTower->findOne($specification, null, $subqueryMap);
		if($kelasTower->issetKelasTowerId())
		{
$appEntityLanguage = new AppEntityLanguageImpl(new KelasTower(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $kelasTower->dateFormatWaktuBuat('j F Y H:i:s');?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpBuat();?></td>
						<td><?php echo $kelasTower->getIpBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminBuat();?></td>
						<td><?php echo $kelasTower->getAdminBuat();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getWaktuUbah();?></td>
						<td><?php echo $kelasTower->dateFormatWaktuUbah('j F Y H:i:s');?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpUbah();?></td>
						<td><?php echo $kelasTower->getIpUbah();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAdminUbah();?></td>
						<td><?php echo $kelasTower->getAdminUbah();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getAktif();?></td>
				...
				<div class="data-wrapper">
					<table class="table table-row table-sort-by-column">
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $kelasTower->getKelasTowerId();?>" data-sort-order="<?php echo $kelasTower->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>" data-active="<?php echo $kelasTower->optionAktif('true', 'false');?>">
								<?php if($userPermission->isAllowedSortOrder()){ ?>
