...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/admin", "pengaturan-pengguna", $appLanguage->getPengaturanPengguna());
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$updater = $admin->where($specification)
		...
		->setKtskId($inputPost->getKtskId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true))
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getEmail();?></td>
						<td>
							<input type="text" class="form-control" name="email" id="email" value="<?php echo $admin->getEmail();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getTelepon();?></td>
						<td>
							<input type="text" class="form-control" name="telepon" id="telepon" value="<?php echo $admin->getTelepon();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getJenisKelamin();?></td>
						...
						<td>
							<select class="form-control" name="ktsk_id" id="ktsk_id">
								...
								PicoSortable::getInstance()
									->add(new PicoSort(Field::of()->sortOrder, PicoSort::ORDER_TYPE_ASC))
									->add(new PicoSort(Field::of()->nama, PicoSort::ORDER_TYPE_ASC)), 
								Field::of()->ktskId, Field::of()->nama, $admin->getKtskId())
								; ?>
							</select>
						</td>
					</tr>
						...
						<td>
							<select class="form-control" name="supervisor_id" id="supervisor_id">
								...
								PicoSortable::getInstance()
									->add(new PicoSort(Field::of()->sortOrder, PicoSort::ORDER_TYPE_ASC))
									->add(new PicoSort(Field::of()->nama, PicoSort::ORDER_TYPE_ASC)), 
								Field::of()->supervisorId, Field::of()->nama, $admin->getSupervisorId())
								; ?>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<select class="form-control" name="lang_id" id="lang_id" data-value="<?php echo $admin->getLangId();?>">
								<option value=""><?php echo $appLanguage->getLabelOptionSelectOne();?></option>
								<option value="id" <?php echo AppFormBuilder::selected($admin->getLangId(), 'id');?>>Indonesia</option>
...
require_once $appInclude->mainAppHeader(__DIR__);
			...
			$mapForLangId = array(
				"id" => array("value" => "id", "label" => "Indonesia", "group" => "", "selected" => true),
