...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$admin->setNama($admin->getNamaDepan() . " " . $admin->getNamaBelakang());
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	try
	{
		...
		// Find if username already used by another user
		$specification2 = PicoSpecification::getInstance()->addAnd(PicoPredicate::getInstance()->like(Field::of()->username, $username))
		->addAnd(PicoPredicate::getInstance()->notEquals(Field::of()->adminId, $newId))
		;

		$admin2 = new Admin(null, $database);
		try
		{
			// Do nothing if found
			$admin2->findOne($specification2);
		}
		catch(Exception $e)
		{
			// Update username if not found
			$specification3 = PicoSpecification::getInstance()->addAnd(PicoPredicate::getInstance()->equals(Field::of()->adminId, $newId));
			$admin2 = new Admin(null, $database);
			$admin2->where($specification3)
