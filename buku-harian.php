...
<script type="text/javascript" src="<?php echo $baseAssetsUrl;?><?php echo $themePath;?>vendors/popper/popper.min.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script type="text/javascript" src="<?php echo $baseAssetsUrl;?><?php echo $themePath;?>vendors/bootstrap/js/bootstrap.min.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script type="text/javascript" src="<?php echo $baseAssetsUrl;?><?php echo $themePath;?>vendors/bootstrap/js/bootstrap5.bundle.min.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script type="text/javascript" src="<?php echo $baseAssetsUrl;?><?php echo $themePath;?>vendors/summernote/0.8.20/summernote.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script type="text/javascript" src="<?php echo $baseAssetsUrl;?><?php echo $themePath;?>vendors/summernote/0.8.20/summernote-bs4.min.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<style>
	.note-hint-popover {
		position: absolute;
	}
...
<script type="text/javascript" src="lib.assets/mobile-script/buku-harian.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script type="text/javascript" src="lib.assets/mobile-script/buku-harian-editor.js?rand=<?php echo mt_rand(1, 99999999);?>"></script>
<script>
	var elements = [];
	jQuery(function($) {
		let editors = [];
...
<div class="page page-jambi page-insert">
	...
	<div class="modal modal-lg fade" id="man-power-modal" tabindex="-1" aria-labelledby="manPowerLabel" aria-hidden="true" data-proyek-id="<?php echo $proyekId;?>">
				...
				<tbody>
								...
								?>
									...
									<select class="resource-man-power form-control" required="required" name="man_power_id[<?php echo $dataIndex;?>]" data-value="<?php echo $manPowerProyek->getManPowerId();?>">
										<?php
											...
											{
												?>
												<option value="<?php echo $manPower->getManPowerId();?>"<?php echo $manPower->getManPowerId() == $manPowerProyek->getManPowerId() ? ' selected' : '';?>><?php echo $manPower->getNama();?></option>
												<?php
											}
										?>
									</select>
									...
									<td>
									<input type="number" step="any" min="1" value="<?php echo $manPowerProyek->getJumlahPekerja();?>" class="form-control" required="required" name="jumlah_man_power[0]">
									</td>
									<td>
					...
					<tr>
						...
						<table class="tabel-control" id="tabel-material" cellpadding="0" cellspacing="0" border="0" class="form-control">
							...
							try
							{
								$materialFinder = new Material(null, $database);
								$dt1 = $materialFinder->findByAktif(true);
								...
								{		
									...
									while($materialProyek = $pageData->fetch())
									{
										...
										<?php
											foreach($dt1->getResult() as $material)
											{
												?>
												<option value="<?php echo $material->getManPowerId();?>"<?php echo $material->getMaterialId() == $materialProyek->getMaterialId() ? ' selected' : '';?>><?php echo $material->getNama();?> [<?php echo $material->getSatuan();?>]</option>
												<?php
											}
										?>
									</select>
									...
									<td>
									<input type="number" step="any" min="0" value="1" class="form-control" required="required" name="jumlah_material[0]">
									</td>
									<td>
										...
										<tr data-value="<?php echo $peralatanProyek->getPeralatanId();?>">
											<td>
												...
												<?php
													...
													{
														?>
														<option value="<?php echo $peralatan->getPeralatanId();?>"<?php echo $peralatan->getMaterialId() == $peralatanProyek->getPeralatanId() ? ' selected' : '';?>><?php echo $peralatan->getNama();?> [<?php echo $peralatan->getSatuan();?>]</option>
														<?php
													}
												?>
											</select>
											...
											<td>
											<input type="number" step="any" min="0" value="1" class="form-control" required="required" name="jumlah_peralatan[0]">
											</td>
											<td>
...
<div class="page page-jambi page-detail">
	<div class="jambi-wrapper">
		...
		<link rel="stylesheet" type="text/css" href="lib.assets/mobile-style/buku-harian.css">
		<script type="text/javascript" src="lib.assets/mobile-script/buku-harian.js?rand=<?php echo mt_rand(1, 99999999);?>">
		</script>
		<script type="text/javascript">
					...
					$(document).on('change', '#file-upload', function(e) {
						...
						if(fileInput.files.length > 0) {
							...
							if (fileExtension === 'jpg' || fileExtension === 'jpeg') {
								...
								$.ajax({
									url: 'lib.mobile-tools/ajax-galeri-proyek.php', // PHP script that handles the file upload
									type: 'POST',
									data: formData,
									contentType: false,
									processData: false,
