...
function mobileUI(resources, opt)
{
	...
	this.init = function(resources){
		...
		$(document).on('mouseover', '[data-rel="tooltip"]', function(e){
		...
		$(document).on('mouseout', '[data-rel="tooltip"]', function(e){
			var _trigger = this;
			var _section = $(_trigger).attr('href').substr(1);
		...
		/*popup start*/
		$(document).on('click', '[data-role="navbar"][data-mark-visited="true"] a', function(e){
		...
		$(document).on('click', '[data-rel="popup"]', function(e){
			...
			// data-no-escape
			if(typeof that.attr('data-no-escape') != 'undefined')
			{
				_noesc = that.attr('data-no-escape');
				$('[data-role="popup"][data-section="'+_section+'"]').attr('data-no-escape', _noesc);
			}
			thisPlugin.showPopUp(_section);
			return false;
		
		});
		$(document).off('click', '[data-dismiss="popup"]');
		$(document).on('click', '[data-dismiss="popup"]', function(e){
			e.preventDefault();
			thisPlugin.closePopUp($(this).closest('[data-role="popup"]').attr('data-section'));
		});
		...
		/*popup end*/
		$(document).off('click', '[data-dismiss="aside"]');
		$(document).on('click', '[data-dismiss="aside"]', function(e){
			e.preventDefault();
			thisPlugin.closeAside($(this).closest('[data-role="aside"]').attr('data-section'));
		});
		/*scroll-up start*/
		$(document).off('click', '[data-rel="scroll-up"]');
		$(document).on('click', '[data-rel="scroll-up"]', function(e){
			$('html, body').animate({
				scrollTop: 0
		...
		$(document).on('click', '[data-rel="scroll-down"]', function(e){
		...
		$(document).on('click', '[data-rel="forward"]', function(e){
			window.history.forward();
			return false;
		});
		/*forward end*/
		$(document).off('click', '[data-role="navbar"][data-rel="swipe-page"] a'); 
		$(document).on('click', '[data-role="navbar"][data-rel="swipe-page"] a', function(e){
			// set siblings
			var that1 = $(this);
		...
		$(document).on('change', '[data-rel="dynamic-select"]', function(e){
		...
		$(document).on('change', '[data-rel="check-all"]', function(e){
			var _that = $(this);
		...
		$(document).on('click', '[data-rel="ajax-get"]', function(e){
			var _this = $(this);
		...
		$(document).on('keyup', '[data-role="autocomplete"]', function(e){
			if(e.which == 38)
			{
				// arrow up
				e.preventDefault();
		...
		$(document).off('keydown', '[data-role="autocomplete"]');
		$(document).on('keydown', '[data-role="autocomplete"]', function(e) {
		...
		$(document).off('blur', '[data-role="autocomplete"]');
		$(document).on('blur', '[data-role="autocomplete"]', function(e) {
			var obj = $('.mui-autocomplete');
			setTimeout(function(){
			obj.fadeOut(100, function(el){
					$(this).remove();
				});
			}, 100);
		});
		...
		if($.inArray('swipe-list', arrres) != -1)
		{
			$('[data-role="swipe-list"]').each(function(index){
				...
				$(document).on('click', '[data-role="swipe-list"] > li > div > a', function(e){
		...
		if($.inArray('aside', arrres) != -1)
		{
			...
			$('aside[data-transition="left"]').each(function(index){
				...
				_that.css({'margin-left':'-'+_width+'px', 'display':'none'});
			});
		...
		$(document).on('click', '[data-role="tab"] > ul > li > a', function(e){			
			var that = $(this);
			var _url = that.attr('href');
			if(_url.indexOf('#') == 0)
			_url = _url.substr(1);
		...
		$(document).on('click', '[data-role="tab"] > [data-role="tab-nav"] > a', function(e){			
			var that = $(this);
			var _url = that.attr('href');
			if(_url.indexOf('#') == 0)
			_url = _url.substr(1);
		...
		{
			$('[data-role="collapsible"]').each(function(e){
				var that = $(this);
				that.find('[data-role="collapsible-header"] h3').prepend('<span class="icon sign-plus"></span><span class="icon sign-minus"></span> ');
								
			});
		}
		$(document).on('click', '[data-role="collapsible"] [data-role="collapsible-header"] a', function(e){
			var par = $(this).closest('[data-role="collapsible"]');
		...
		if($.inArray('accordion', arrres) != -1)
		{
			...
			$('[data-role="accordion"]').each(function(e){
				var that = $(this);
				...
				that.find('[data-role="accordion-section"] h3:nth-child(1) a').on('click', function(e){
					var obj = $(this).closest('[data-role="accordion-section"]');
					...
					if(_nav == 'true')
					{
						that.find('[data-role="accordion-content"] a').each(function(index2){
							...
							if(dl.indexOf(_url) != -1 || dl == _url)
							{
								...
								that2.closest('[data-role="accordion-content"]').css({'display':'block'});
								var sec = that2.closest('[data-role="accordion-section"]');
		...
		if($.inArray('slideshow', arrres) != -1)
		{
			$('[data-role="slideshow"]').each(function(index){
				...
				that.attr({'data-active-slide':_active, 'data-num-slide':_num});
				...
				that.find('.data-slideshow-nav-next a').on('click', function(e){
					var _width = parseFloat(that.width());
				...
				that.find('.data-slideshow-nav-prev a').on('click', function(e){
					var _width = parseFloat(that.width());
					var _cs = parseInt(that.attr('data-active-slide')) || 1;
					var _fs = _cs;
					var _ss = _cs-1;
		...
		$('[data-filter="true"]').each(function(index){
			...
			.attr({'data-filter-id':'filter'+index, 'placeholder':placeholder})
			.insertBefore(that);			
		...
		$('[data-sort="true"]').each(function(index){
			...
			for(i in arrc)
			{
				...
				obj.on('click', function(e){
					...
					that2.siblings().attr('data-active', 'false');
					that2.attr('data-active', 'true');
	...
	this.showPopUp = function(_section)
	{
		...
		if(_cls)
		{
			_close = '<a href="#" data-dismiss="popup" data-position="float-right"><span class="icon remove"></span></a> ';
		}
		if(_min)
		{
			_minimize = '<a href="#" data-dismiss="popup" data-position="float-right"><span class="icon minus"></span></a> ';
		}
		if(title!='')
		{
			domObj.find('[data-role="header"]').remove();
			domObj.prepend('<h3 data-role="header">'+_close+_minimize+title+'</h3>');
			if(_cls)
			{			
			}
		}
		if(_modal)
		{
			...
			$('<div />').attr({'data-role':'overlay','data-section':_section}).css({'z-index':zIndex}).insertBefore(domObj).fadeIn(thisPlugin.animDelay);
		...
		if(typeof ajaxLocation != 'undefined')
		{
			...
			$.ajax({
				...
				success:function(data, textStatus, XMLHttpRequest){
					domObj.empty().append(data);
					if(title!='')
					{
						domObj.find('[data-role="header"]').remove();
						domObj.prepend('<h3 data-role="header">'+_close+title+'</h3>');
						if(_cls)
						{			
	...
	this.closeUpperPopUp = function(){
		...
		var noEsc = $('[data-role="popup"][data-section="'+section+'"]').attr('data-no-escape') || 'false';
		if(noEsc == 'false')
		{
	...
	this.alert = function(message, okButton)
	{
		if($('[data-section="custom-alert"]').length)
		{
			$('[data-section="custom-alert"]').remove();
		}
		message = message || '';
		message = message.replace(/(?:\r\n|\r|\n)/g, '<br />');
		okButton = okButton || 'OK';
		var html = '<div id="custom-alert" data-role="popup" data-section="custom-alert" data-modal="true"><div data-role="message-body">'+message+'</div><div data-role="footer"><button id="custom-button-ok" data-dismiss="popup">'+okButton+'</button></div></div>';
		$('body').append(html);
		mui.showPopUp('custom-alert');
		$('#custom-button-ok').focus();
	}
	this.confirm = function(message, onOKFunction, onCancelFunction, okButton, cancelButton, noEscape)
	{
		if($('[data-section="custom-confirm"]').length)
		{
			$('[data-section="custom-confirm"]').remove();
		}
		message = message || '';
		message = message.replace(/(?:\r\n|\r|\n)/g, '<br />');
		okButton = okButton || 'OK';
		cancelButton = cancelButton || 'Cancel';
		var lock = (noEscape)?' data-no-escape="true"':'';
		var html = '<div id="custom-confirm" data-role="popup" data-section="custom-confirm" data-modal="true"'+lock+'><div data-role="message-body">'+message+'</div><div data-role="footer"><button id="custom-button-ok">'+okButton+'</button> <button id="custom-button-cancel">'+cancelButton+'</button></div></div>';
		$('body').append(html);
		mui.showPopUp('custom-confirm');
		$('[data-section="custom-confirm"] #custom-button-ok').on('click', function(){
			if(typeof onOKFunction == 'function')
			{
				onOKFunction();
			}
			mui.closePopUp('custom-confirm');
			$('[data-section="custom-confirm"]').remove();
		});
		$('[data-section="custom-confirm"] #custom-button-cancel').on('click', function(){
	...
	this.prompt = function(message, value, onOKFunction, onCancelFunction, okButton, cancelButton, noEscape)
	{
		if($('[data-section="custom-prompt"]').length)
		{
			$('[data-section="custom-prompt"]').remove();
		}
		value = value || '';
		message = message || '';
		message = message.replace(/(?:\r\n|\r|\n)/g, '<br />');
		...
		cancelButton = cancelButton || 'Cancel';
		var lock = (noEscape)?' data-no-escape="true"':'';
		var html = '<div id="custom-prompt" data-role="popup" data-section="custom-prompt" data-modal="true"'+lock+'><div data-role="message-body">'+message+'<div data-role="input-area"><input type="text" name="custom-prompt" data-full-width="true"></div></div><div data-role="footer"><button id="custom-button-ok">'+okButton+'</button> <button id="custom-button-cancel">'+cancelButton+'</button></div></div>';
		$('body').append(html);
		mui.showPopUp('custom-prompt');
		$('input[name="custom-prompt"]').val(value);
		$('[data-section="custom-prompt"] #custom-button-ok').on('click', function(){
			var val = $('input[name="custom-prompt"]').val();
			if(typeof onOKFunction == 'function')
			{
				onOKFunction(val);
			}
			mui.closePopUp('custom-prompt');
			$('[data-section="custom-prompt"]').remove();
		});
		$('[data-section="custom-prompt"] #custom-button-cancel').on('click', function(){
			var val = $('input[name="custom-prompt"]').val();
			if(typeof onCancelFunction == 'function')
			{
				onCancelFunction(val);
			}
			mui.closePopUp('custom-prompt');
			$('[data-section="custom-prompt"]').remove();
		});
