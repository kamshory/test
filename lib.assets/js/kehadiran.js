...
function triggerDetectLocation() {
    document.querySelector('#captureButton').disabled = true;
    document.querySelector('#clearButton').disabled = true;
    document.querySelector('#uploadButton').disabled = true;
    detectGeolocation(function(latitude, longitude) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelector('[name="jenis_kehadiran"]').addEventListener('change', function(e){
        if(e.target.value == 'P')
        {
            document.querySelector('[name="aktivitas"]').style.display = 'block';
        }
        else
        {
            document.querySelector('[name="aktivitas"]').style.display = 'none';
        }
    });

    document.getElementById('uploadButton').addEventListener('click', function(e) {

        let jenisKehadiran = $('[name="jenis_kehadiran"]').val();
        let lokasiId = $('[name="lokasi_id"]').val();
        let aktivitas = $('[name="aktivitas"]').val();
        let pass = true;

        if(jenisKehadiran == '')
        {
            $('[name="jenis_kehadiran"]')[0].focus();
            pass = false;
            e.preventDefault();
            return false;
        }
        ...
        if(lokasiId == '')
        {
            $('[name="lokasi_id"]')[0].focus();
            pass = false;
            e.preventDefault();
            return false;
        }

        if(jenisKehadiran == 'P' && aktivitas == '')
        {
            $('[name="aktivitas"]')[0].focus();
            pass = false;
            e.preventDefault();
            return false;
        }

        if(!confirm('Apakah Anda yakin akan mengirimkannya?'))
        {
            return false;
        }
        ...
        if(pass)
        {
            croppedCanvas.toBlob((blob) => {
                const formData = new FormData($('#form-kehadiran')[0]);
                ...
                // Set up the request
                xhr.open('POST', 'kehadiran.php', true);
        
                // Handle response
                xhr.onload = () => {
                    if (xhr.status === 200) {
                        window.location = 'kehadiran.php';
                    } else {
